...
Returns everything an agent should follow **before** authoring HTML. No auth. Response `data` keys: `theme`, `version`, `tokens` (CSS custom properties), `rules` (hard constraints), `components` (ready-to-paste snippets — slides, KPI cards, callouts, charts, figures, galleries, …), and `shells` (`{ deck, report }` document skeletons); a top-level `themes` lists the available themes, and `recommended` names the default per document form. Two themes ship today: **`prose`** (Quiet Editorial — Newsreader serif on a warm paper surface; the default for markdown `report` publishes and the usual pick for reports) and **`vibrant`** (Midnight Azure — ink-violet headings, azure accents; deck-first display styling, the default for decks and raw-HTML publishes). Pass `?theme=` here and the same `theme` on publish so the spec you authored against is the stylesheet you ship. The design system is **dark-mode aware**: documents adapt to the reader's `prefers-color-scheme`, and authors can force an ink slide with `class="slide dark"`.
